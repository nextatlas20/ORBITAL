...
The preview train packages the telemetry refactor and the command-line installer. This note is intentionally kept separate from the generated register.

### 🧭 Release TODO Register
...
- [ ] **orbital/cli.py:2** - add shell completion for the profile command
- [ ] **orbital/cli.py:4** - validate the manifest before upload
- [ ] **orbital/engine/dispatch.py:1** - record retry reasons in the event envelope
- [ ] **orbital/engine/dispatch.py:2** - preserve ordering when batches are replayed
- [ ] **orbital/telemetry/exporter.py:1** - expose a bounded queue size
- [ ] **orbital/telemetry/exporter.py:2** - redact tenant identifiers in debug output

## Verification notes

The register is generated from Python source markers in the preview checkout. Markdown examples and archived notes are not part of its input.
